  ...
  <div class="site-shell">
    ...
    <main id="main">
      ...
      <template v-if="route === '/'">
        ...
        <a class="study-card" href="#/studies/gpt6-components"><div class="project-mark" aria-hidden="true">6<span>↗</span></div><div class="project-copy"><div class="tag-row"><span class="pill">{{ labels[study?.state ?? 'no_data'] }}</span><span class="subtle">原始事实 · 联合分析</span></div><h3>GPT-6 额度异常归因</h3><p>整体倍率？缓存读取？还是多项混合？比较七类解释。</p><div class="project-numbers"><span><b>{{ loading ? '—' : n(study?.totals.requests) }}</b>贡献请求</span><span><b>{{ loading ? '—' : n(study?.totals.contributors) }}</b>贡献安装</span><span><b>7</b>候选解释</span></div></div><span class="project-arrow" aria-hidden="true">↗</span></a>
