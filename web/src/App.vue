...
const ordered = computed(() =>
  [...(study.value?.causes ?? [])].sort((a, b) => (b.support ?? -1) - (a.support ?? -1)),
);
...
const qualityEntries = computed(() =>
  Object.entries(study.value?.quality ?? {}).filter(([, v]) => v > 0),
);
  ...
  <div class="site-shell">
    ...
    <main id="main">
      ...
      <template v-if="route === '/'">
        <section class="hero">
          <div class="hero-copy">
            <h1>让每一份用量，<br />成为一份<span class="highlight">证据。</span></h1>
            <div class="hero-actions">
              <a class="button primary" href="#/studies/gpt6-components">进入 GPT-6 研究 ↗</a
              ><a class="text-link" href="#/privacy">了解如何参与 →</a>
            </div>
          </div>
          <div class="hero-art" aria-hidden="true">
            <div class="art-note">先提出假设<br /><strong>再让数据说话</strong></div>
            <div class="art-stack mint"><span>01</span><strong>输入 / 缓存 / 输出</strong></div>
            <div class="art-stack violet"><span>02</span><strong>小片段也能互相补充</strong></div>
            <div class="art-stack peach"><span>03</span><strong>保留不确定性。</strong></div>
            <div class="art-plus">＋</div>
          </div>
        </section>
        <section class="section-head">
          <h2>正在进行的研究</h2>
          <span class="pill plain">02 个项目</span>
        </section>
        <a class="study-card" href="#/studies/gpt6-components"
          ><div class="project-mark" aria-hidden="true">6<span>↗</span></div>
          <div class="project-copy">
            <div class="tag-row">
              <span class="pill">{{ labels[study?.state ?? "no_data"] }}</span>
            </div>
            <h3>GPT-6 额度异常归因</h3>
            <p>整体倍率？缓存读取？还是多项混合？比较七类解释。</p>
            <div class="project-numbers">
              <span
                ><b>{{ loading ? "—" : n(study?.totals.requests) }}</b
                >贡献请求</span
              ><span
                ><b>{{ loading ? "—" : n(study?.totals.contributors) }}</b
                >贡献安装</span
              ><span><b>7</b>候选解释</span>
            </div>
          </div>
        ...
        <section class="principles">
          <article>
            ...
            <h3>明细留在本地</h3>
            <p>不接收请求文本、Token 明细、账号身份或容量估值。</p>
          </article>
          <article>
            <span class="mini-icon violet">≈</span>
            <h3>小证据，共同研究</h3>
            <p>不要求每个人先得到结论；不同的少量信息也可以互相补充。</p>
          </article>
          <article>
            <span class="mini-icon peach">↻</span>
            <h3>更新去重，批次保留</h3>
            <p>同一批次更新替换，不同批次保留，不因停止更新而自动删除。</p>
          </article>
        </section>
      </template>
      ...
      <template v-else-if="detail">
        ...
        <section class="detail-heading">
          ...
          <div class="refresh-block">
            <span class="pill">{{ labels[study?.state ?? "no_data"] }}</span
            ><button class="button small" :disabled="refreshing" @click="load">
        ...
        <template v-else-if="study">
          <section class="metrics" aria-label="科研数据总量">
            ...
            <article class="mint">
              <span>贡献请求总量</span
              ><strong>{{ n(study.totals.requests) }}<small>次</small></strong>
              <p>
            ...
            <article class="peach">
              ...
              <p>
                {{ n(study.totals.intervals) }} 个原始区间 /
                {{ n(study.totals.contrasts) }} 个容量内对比
              </p>
            </article>
          </section>
          ...
          >
            ...
            <div>
              ...
              <p>
                一条请求也能贡献，无需满两个周期或 200 次请求。没有区分信息时不把先验偏好冒充发现。
              </p>
              ...
              <article class="panel factor-panel">
                ...
                <div class="factor-grid">
                  <div v-for="(value, index) in selectedCause?.factor_estimates" :key="index">
                    ...
                    ><strong>{{
                      selectedCause?.support == null ? "—" : `× ${value.toFixed(2)}`
                    }}</strong>
                  </div>
                </div>
                <p class="fineprint">
                  联合证据在该解释族内的倍率均值，不是当前网关价格。没有读取或使用
                  PF／平均恒定容量估值。
                </p>
              </article>
            </aside>
      ...
      <section v-else-if="route === '/method'" class="prose-page">
        <span class="eyebrow">METHOD / RAW FACTS</span>
        <h1>小证据，也能<br />共同回答大问题。</h1>
        <p class="lead">
          每个人不必先独立识别原因；客户端提交同一组候选倍率的统计证据，中心联合分析。
        </p>
        <div class="method-steps">
          <article class="panel mint">
            ...
            <h2>固定其他计费因素</h2>
            <p>
              FAST 正确目标为 2，GPT-5.6／GPT-6
              长上下文不额外翻倍，其他模型价格假定正确。本项目只估计 GPT-6 四分项倍率。
            </p>
          </article>
          <article class="panel violet">
            <span class="step">02</span>
            <h2>只用原始事实</h2>
            <p>
              按账号短片段处理未知容量尺度，考虑额度整数显示和波动。不发送、不存储、不借用粒子滤波或平均恒定容量估值。
            </p>
          </article>
          <article class="panel peach">
            ...
            <p>
              1,311
              组共同倍率参数，先累加证据，再按七类解释归一化。局部缺少某个方向的信息，可以由其他来源补充。
            </p>
          </article>
        </div>
        <div class="panel">
          <h2>哪些数据进入研究？</h2>
          <p>
            无本地请求数、区间数、周期数或置信度门槛。FAST、长上下文、混用其他模型不会触发整段拒收。缺少对应额度或成本分项时仍接收规模统计，但不会制造不存在的证据。
          </p>
          <h2>支持度有什么含义？</h2>
          <p>
            支持度是预先固定方法、价格前提与容量工作模型下的条件证据分布，不是官方真实机制的已校准概率。独立、互补的小样本能补充信息；所有来源共有的系统性偏差不会自动消失。
          </p>
          <h2>如实保留失败情景</h2>
          <p>
            容量突变系统性追随模型选择时，仍可能产生错误的高支持度；方法文档保留反例，不把软件测试通过说成真实计费研究成功。
          </p>
          <a
            class="button"
            href="https://github.com/LingyeNBird/CodexSubscribeStudy/blob/main/docs/method.md"
            target="_blank"
            rel="noreferrer"
            >公式、实验与限制 ↗</a
          >
      ...
      <section v-else-if="route === '/privacy'" class="prose-page">
        <span class="eyebrow">PRIVACY / VOLUNTARY CONTRIBUTIONS</span>
        <h1>参与是选择。<br />知情是前提。</h1>
        <p class="lead">
          Sub2Pool 默认关闭科研，保存并明确授权后才分享。接收网站默认为
          https://codex.nightunderfly.online/。
        </p>
        <div class="privacy-grid">
          <article class="panel mint">
            <span class="pill plain">会发送</span>
            <h2>原始事实的统计证据</h2>
            <p>
              批次请求数、原始金额和额度统计、质量计数、共同参数证据曲线、信息矩阵及候选额度归属函数。随机安装公钥和批次编号用于去重和更新。
            </p>
          </article>
          <article class="panel violet">
            <span class="pill plain">不会发送</span>
            <h2>请求与身份明细</h2>
            <p>
              提示词、回答、Token 明细、调用时间线、账号／参与者名称、API Key、Sub2API 地址、IP
              字段，以及粒子滤波／平均恒定容量估值或其辅助分析。
            </p>
          </article>
        </div>
        <div class="panel">
          <h2>“匿名”有明确边界</h2>
          <p>
            随机公钥具有同站点链接性，因此是去标识化，不是绝对匿名。无门槛意味着小贡献也可能有较少聚合保护。网络层和反代仍能看到出口
            IP，应用不记录 IP 不代表网络设施不可见。
          </p>
          <h2>贡献会长期保留</h2>
          <p>
            同批次更新替换，不同批次长期保留，不因 120
            天未更新、停止参与、关闭科研、导入数据库或身份失效而删除。
          </p>
          <p>
            本站不提供删除已提交贡献的接口。参与前请确认接受长期保留规则。不同随机身份之间无法自动辨认重复的同一额度池。
          </p>
          <h2>面向开发者的公共接口</h2>
          <pre>
GET  /api/studies
...
POST /api/reports</pre>
          <a
            class="button"
            href="https://github.com/LingyeNBird/CodexSubscribeStudy/blob/main/docs/api.md"
            target="_blank"
            rel="noreferrer"
            >提交协议与隐私清单 ↗</a
          >
