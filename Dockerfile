...
FROM alpine:3.22
RUN addgroup -g 10001 study && adduser -D -u 10001 -G study study && mkdir /data /config && chown study:study /data
COPY --from=backend /study /usr/local/bin/study
USER 10001:10001
ENV STUDY_ADDR=:8080 STUDY_DB=/data/study.sqlite STUDY_LEGACY_DB=/data/study.db STUDY_ADMIN_CONFIG=/config/admin.json
VOLUME ["/data"]
EXPOSE 8080
